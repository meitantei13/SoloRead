...
統計・分析ページでは、ユーザーの読書履歴を振り返ることができます。
...
読了日の入力には `@mui/x-date-pickers` の `DesktopDatePicker` を使用していますが、カレンダーのヘッダー表示を「2025年 / 1月」という形式にする必要がありました。

当初のバージョンでは `calendarHeader` に `format` プロパティが存在せず、表示形式を変えられないことが分かりました。
CSS による上書きも検討しましたが、MUI の内部実装に依存した方法は安定性に欠けると判断しました。

そこで、MUI のバージョンをアップグレードし、`slotProps.calendarHeader.format` に `"YYYY年 / M月"` を指定することで要件を満たすことができました。

実装にあたっては、扱うデータ型の変換にも注意が必要でした。
`DesktopDatePicker` は `dayjs` オブジェクトを扱う一方で、`react-hook-form` や API 通信では文字列（String）としてデータを扱う必要がありました。
そのため、カレンダー（dayjs オブジェクト）→ フォーム（String）→ API（String）→ Rails（Date）という変換の流れを整理し、`console.log` やブラウザの開発者ツールを用いて値を確認しながら実装を進めました。

この経験から、ライブラリのバージョンによって API の仕様が変わることを意識しながら、公式ドキュメントを確認する重要性を学びました。
