...
function GenreDialog({ open, onClose, onSuccess }: GenreDialogProps) {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const response = await axios.post(genreUrl, { genre: { name: genreName } }, { headers })
    ...
    <Dialog onClose={handleClose} open={open}>
        ...
        <DialogContent>
            <form onSubmit={handleSubmit} id="subscription-form">
                <TextField autoFocus required value={genreName} id="genreId" name="genreName" onChange={(e) => setGenreName(e.target.value)} />
            </form>
        </DialogContent>
        <DialogActions>
            <Button onClick={handleClose}>戻る</Button>
            <Button type="submit" form="subscription-form">追加</Button>
        </DialogActions>
    </Dialog>
  )
}
